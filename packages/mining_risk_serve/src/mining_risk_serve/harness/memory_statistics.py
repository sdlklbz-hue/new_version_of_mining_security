...
def detect_risk_type(text: str, metadata: Optional[Dict[str, Any]] = None) -> str:
    ...
    detect risk type。
        ...
        Args:
            text (str): 参数 ``text``。
            metadata (Optional[Dict[str, Any]]): 参数 ``metadata``。
...
def _connect_readonly(db_path: str) -> sqlite3.Connection:
    """内部辅助方法 ``_connect_readonly``；参数与返回值见类型注解。"""
...
def read_agentfs_file_raw(db_path: str, path: str) -> Tuple[Optional[bytes], Optional[Dict[str, Any]]]:
    """
    read agentfs file raw。

        Args:
            db_path (str): 参数 ``db_path``。
            path (str): 参数 ``path``。

        Returns:
            (Tuple[Optional[bytes], Optional[Dict[str, Any]]]): 函数返回值。
    """
...
def list_agentfs_metadata(db_path: str, root: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    list agentfs metadata。

        Args:
            db_path (str): 参数 ``db_path``。
            root (Optional[str]): 参数 ``root``。
...
def _empty_priority_count() -> Dict[str, int]:
    ...
    Returns:
        Dict[str, int]: 键为 PRIORITIES 中定义的优先级名称。
    """
...
def _date_bucket(timestamp: Optional[float]) -> str:
    """内部辅助方法 ``_date_bucket``；参数与返回值见类型注解。"""
...
def collect_short_term(short_term: Optional[ShortTermMemory]) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
    ...
    collect short term。
        ...
        Args:
            short_term (Optional[ShortTermMemory]): 参数 ``short_term``。

        Returns:
            (Tuple[Dict[str, Any], List[Dict[str, Any]]]): 函数返回值。
    """
...
def _read_rag_report() -> Dict[str, Any]:
    """读取 RAG 索引重建报告 JSON（不存在或解析失败时返回空字典）。
...
def record_matches(record: Dict[str, Any], filters: MemoryStatsFilters) -> bool:
    ...
    record matches。
        ...
        Args:
            record (Dict[str, Any]): 参数 ``record``。
            filters (MemoryStatsFilters): 参数 ``filters``。
...
def apply_filters(records: Iterable[Dict[str, Any]], filters: MemoryStatsFilters) -> List[Dict[str, Any]]:
    ...
    apply filters。
        ...
        Args:
            records (Iterable[Dict[str, Any]]): 参数 ``records``。
            filters (MemoryStatsFilters): 参数 ``filters``。
...
def build_trend(records: List[Dict[str, Any]], operation_logs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    build trend。
        ...
        Args:
            records (List[Dict[str, Any]]): 参数 ``records``。
            operation_logs (List[Dict[str, Any]]): 参数 ``operation_logs``。
