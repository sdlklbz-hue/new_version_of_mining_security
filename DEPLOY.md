...
# 开发态推荐（限制 reload 目录，避免 .venv 触发无限重载）
bash scripts/run_api.sh --reload
```

API 文档：http://localhost:8000/docs 。需要 RAG 时额外安装 `requirements-serve-rag.txt` 并执行 `scripts/rebuild_rag_index.py`。

如果要在本地训练模型、运行 RAG/NLP 或全量测试，请安装：
...
| 容器路径 | 宿主机路径 | 说明 |
|---|---|---|
| `/app/var` | `./var` | Chroma 向量库、AgentFS、决策 JSON、审计 SQLite |
| `/app/artifacts` | `./artifacts` | Stacking 模型与 `preprocessing_pipeline.pkl` |
| `/app/datasets` | `./datasets` | 公开数据、企业库、demo 回放批次 |
| `/app/knowledge_base` | `./knowledge_base` | 六库 Markdown 权威正文 |
| `/app/memory` | `./memory` | 长期记忆归档 |
| `/app/logs` | `./logs` | 运行时日志 |
| `/app/packages` | `./packages` | 源码包（开发/热修可选） |
| `/app/config.yaml` | `./config.yaml` | 权威配置 |
...
cp var/agentfs/agentfs.db backup/agentfs_$(date +%Y%m%d).db
cp var/audit/audit.db backup/audit_$(date +%Y%m%d).db

# 备份知识库 Git 仓库
tar czf backup/kb_$(date +%Y%m%d).tar.gz var/agentfs/git knowledge_base
```
...
- **数据路径**：`data.raw_data_path`、`data.merged_data_path`（默认 `datasets/raw/public`、`datasets/interim/merged`）
- **模型参数**：`model.stacking.*`
...
| 模型未训练 → 返回 Mock | 确认 `artifacts/models/` 与 `artifacts/pipelines/` 存在，或先跑 `python scripts/train_model.py` |
...
6. 定期回查 `var/audit/audit.db`，巡检 `iteration` 触发与审批历史。
